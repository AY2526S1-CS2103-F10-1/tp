...
Below is a sequence diagram to illustrate how `ModelManager` filters and displays the list of person through
`PersonList`, which updates the `FilteredList` according to the predicates.

<puml src="diagrams/FindCommandSequenceDiagram.puml" width="900" height="500"/>

<box type="info" seamless>

**Note**: This sequence diagram only focuses on how the `ModelManager` filters the list, it does not include the implementation of findCommand or findCommandParser.
</box>

When `updatePersonListFilter(Predicate<Person>...)` is called, the sequence of updating the person list is:

1. Passing `Predicate<Person>...` into `andAll(Predicate<Person>...)`, combining the predicates into a single predicate
...
**Note**: The constructor details of FilteredList are omitted as they are not essential to understanding the interaction.
</box>
...
2. These commands remain untouched from the original AB3 AddressBook and are hence excluded: _edit, list, clear, delete, help, exit_
...
1. Finding a person by Name
    ...
    1. Prerequisites: At least one person is named: `John` and none named: `Johnd`

    1. Test case: `find n=John`<br>
       Expected: Persons named: `John` is displayed.

    1. Test case: `find n=johnd`<br>
       Expected: No person is displayed. Details shown in the status message.

2. Finding a person by Number

    1. Prerequisites: At least one person has a number: `987654321` and none with a number `9876543210`

    2. Test case: `find mn=987` <br>
       Expected: Persons with number: `987` as substring is displayed.

    3. Test case: `find mn=987654321` <br>
       Expected: Persons with number: `987654321` is displayed.

    4. Test case: `find mn=9876543210` <br>
       Expected: No person is displayed. Details shown in the status message.

3. Finding a person by Tag

    1. Prerequisite: At least one person with a tag: `friend` and none with a tag: `buddy`

    2. Test case: `find t=friend` <br>
        Expected: Persons with tag: `friend` is displayed
   
    3. Test case: `find t=buddy` <br>
        Expected: No person is displayed. Details shown in the status message.
